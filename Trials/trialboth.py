...
def main():
    ...
    target_stock = 'WIPRO.NS'  # Replace with your desired stock from the list above
    ...
    try:
        ...
        # Calculate correlations - only need to do this once
        ...
        if lstm_results is None:
            print("Error during LSTM model training. Skipping LSTM results.")
        else:
            # Display LSTM metrics
            print("\n" + "="*50)
            print(f"LSTM MODEL SUMMARY FOR {target_stock}")
            print("="*50)
            print(f"Cross-validation (Time Series Split):")
            print(f"Average RMSE: ₹{lstm_results['avg_rmse']:.4f}")
            print(f"Average MAPE: {lstm_results['avg_mape']:.4%}")
            print("\nFull Dataset Metrics:")
            print(f"Overall RMSE: ₹{lstm_results['overall_rmse']:.4f}")
            print(f"Overall MAPE: {lstm_results['overall_mape']:.4%}")
            print("\nModel Information:")
            print(f"Number of features used: {lstm_results['n_features']}")
            print(f"Number of samples: {lstm_results['n_samples']}")
            print("\nTop 5 most important features:")
            sorted_features = dict(sorted(
                lstm_results['feature_importance'].items(),
                key=lambda x: x[1],
                reverse=True
            )[:5])
        ...
        if xgb_results is None:
            print("Error during XGBoost model training. Skipping XGBoost results.")
        else:
            # Display XGBoost metrics
            print("\n" + "="*50)
            print(f"XGBOOST MODEL SUMMARY FOR {target_stock}")
            print("="*50)
            print(f"Cross-validation (Time Series Split):")
            print(f"Average RMSE: ₹{xgb_results['avg_rmse']:.4f}")
            print(f"Average MAPE: {xgb_results['avg_mape']:.4%}")
            print("\nFull Dataset Metrics:")
            print(f"Overall RMSE: ₹{xgb_results['overall_rmse']:.4f}")
            print(f"Overall MAPE: {xgb_results['overall_mape']:.4%}")
            print("\nModel Information:")
            print(f"Number of features used: {xgb_results['n_features']}")
            print(f"Number of samples: {xgb_results['n_samples']}")
            print("\nTop 5 most important features:")
            sorted_features = dict(sorted(
                xgb_results['feature_importance'].items(),
                key=lambda x: x[1],
                reverse=True
            )[:5])
            for feature, importance in sorted_features.items():
            ...
            # Performance Metrics and Time Comparison
            print("\nPerformance Metrics and Time Comparison:")
            print("-" * 60)
            print(f"{'Metric':<20} {'LSTM':<15} {'XGBoost':<15}")
            print("-" * 60)
            print(f"{'Overall RMSE (₹)':<20} {lstm_results['overall_rmse']:<15.4f} {xgb_results['overall_rmse']:<15.4f}")
            print(f"{'Overall MAPE (%)':<20} {lstm_results['overall_mape']*100:<15.4f} {xgb_results['overall_mape']*100:<15.4f}")
            print(f"{'Avg CV RMSE (₹)':<20} {lstm_results['avg_rmse']:<15.4f} {xgb_results['avg_rmse']:<15.4f}")
            print(f"{'Avg CV MAPE (%)':<20} {lstm_results['avg_mape']*100:<15.4f} {xgb_results['avg_mape']*100:<15.4f}")
            print(f"{'Training Time (s)':<20} {lstm_training_time:<15.2f} {xgb_training_time:<15.2f}")
            
            # Speed Comparison
            faster_model = "LSTM" if lstm_training_time < xgb_training_time else "XGBoost"
            speed_diff = abs(lstm_training_time - xgb_training_time)
            speed_ratio = max(lstm_training_time, xgb_training_time) / min(lstm_training_time, xgb_training_time)
            
            print(f"\nSpeed Analysis:")
            print(f"- {faster_model} was faster by {speed_diff:.2f} seconds")
            print(f"- {faster_model} was {speed_ratio:.2f}x faster than {'XGBoost' if faster_model == 'LSTM' else 'LSTM'}")

            # Top Feature Comparison
            print("\nTop Feature Comparison:")
            print("-" * 40)
            print(f"{'Rank':<6} {'LSTM Feature':<20} {'XGBoost Feature':<20}")
            print("-" * 40)
            
            lstm_top = sorted(lstm_results['feature_importance'].items(), key=lambda x: x[1], reverse=True)[:5]
            xgb_top = sorted(xgb_results['feature_importance'].items(), key=lambda x: x[1], reverse=True)[:5]
            
            for i in range(5):
                print(f"{i+1:<6} {lstm_top[i][0]:<20} {xgb_top[i][0]:<20}")
            
            # Model Selection Recommendation
            print("\nModel Selection Recommendation:")
            print("-" * 40)
            
            if lstm_results['overall_rmse'] < xgb_results['overall_rmse']:
                rmse_better = "LSTM"
                rmse_margin = ((xgb_results['overall_rmse'] - lstm_results['overall_rmse']) / 
                             lstm_results['overall_rmse'] * 100)
            else:
                rmse_better = "XGBoost"
                rmse_margin = ((lstm_results['overall_rmse'] - xgb_results['overall_rmse']) / 
                             xgb_results['overall_rmse'] * 100)
                
            if lstm_results['overall_mape'] < xgb_results['overall_mape']:
                mape_better = "LSTM"
                mape_margin = ((xgb_results['overall_mape'] - lstm_results['overall_mape']) / 
                             lstm_results['overall_mape'] * 100)
            else:
                mape_better = "XGBoost"
                mape_margin = ((lstm_results['overall_mape'] - xgb_results['overall_mape']) / 
                             xgb_results['overall_mape'] * 100)
            
            print(f"RMSE: {rmse_better} performs better by {rmse_margin:.2f}%")
            print(f"MAPE: {mape_better} performs better by {mape_margin:.2f}%")
            
            # Overall Recommendation
            print("\nFinal Recommendation:")
            if lstm_results['overall_rmse'] < xgb_results['overall_rmse'] and lstm_results['overall_mape'] < xgb_results['overall_mape']:
                print("LSTM is recommended for this prediction task (better on both metrics)")
            elif xgb_results['overall_rmse'] < lstm_results['overall_rmse'] and xgb_results['overall_mape'] < lstm_results['overall_mape']:
                print("XGBoost is recommended for this prediction task (better on both metrics)")
            else:
                print("Consider using both models in ensemble as they show different strengths")
                if lstm_results['overall_rmse'] < xgb_results['overall_rmse']:
                    print("- LSTM shows better accuracy in terms of RMSE")
                else:
                    print("- XGBoost shows better accuracy in terms of RMSE")
                if lstm_results['overall_mape'] < xgb_results['overall_mape']:
                    print("- LSTM shows better accuracy in terms of MAPE")
                else:
                    print("- XGBoost shows better accuracy in terms of MAPE")
            
            # Computational Efficiency
            print("\nComputational Efficiency:")
            print(f"- LSTM Training Time: {lstm_training_time:.2f} seconds")
            print(f"- XGBoost Training Time: {xgb_training_time:.2f} seconds")
            if abs(lstm_training_time - xgb_training_time) > 10:  # If difference is significant (>10 seconds)
                print(f"- Consider using {faster_model} if training time is a constraint")
